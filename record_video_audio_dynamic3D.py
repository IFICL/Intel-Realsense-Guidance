...
def main():
    ...
    # create folder
    parent_folder = 'NewAudio3D-Videos/RawData'

    building_folder = os.path.join(parent_folder, args.building)
    os.makedirs(building_folder, exist_ok=True)

    # check number of clip exists
    clip_list = glob.glob(f"{building_folder}/*")
    clip_list.sort()
    clip_id = len(clip_list)
    print(f"Number of clips in {building_folder}: {clip_id}")

    if args.overwrite and clip_id > 0:
        clip_id -= 1
    
    sample_folder = os.path.join(building_folder, 'scene-' + str(clip_id).zfill(3))
...
# Example: 
# python record_video_audio_dynamic3D.py --building='home' --seconds=20 --FPS=15
