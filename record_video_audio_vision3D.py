...
parser.add_argument('--room', type=str, default='str', help='saved folder of .bag and .wav')
...
parser.add_argument('--ifobstacle', type=int, default=1, help='label')
...
def main():
    ...
    # create folder
    parent_folder = 'NewVision3D-Videos/RawData'

    room_folder = os.path.join(parent_folder, args.room)
    os.makedirs(room_folder, exist_ok=True)

    # check number of clip exists
    clip_list = glob.glob(f"{room_folder}/*")
    clip_list.sort()
    clip_id = len(clip_list)
    print(f"Number of clips in {room_folder}: {clip_id}")

    if args.overwrite and clip_id > 0:
        clip_id -= 1
    
    sample_folder = os.path.join(room_folder, 'scene-' + str(clip_id).zfill(3))
...
# Example: 
# python record_video_audio_vision3D.py --room='home' --seconds=20 --FPS=15
